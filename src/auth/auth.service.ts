...
export class AuthService {
  ...
  async verifyEmail(email: string, otp: string): Promise<{ message: string }> {
    ...
    await this.otpService.verify(user.id, email, otp, OtpPurpose.REGISTER);
  ...
  async resendVerificationOtp(email: string): Promise<{ message: string }> {
    ...
    if (!user) {
      return {
        message:
          'If the email exists and is unverified, a verification OTP has been sent.',
      };
    }
  ...
  async login(
    ...
    const roles =
      user.userRoles?.map((r) => r.role?.name).filter(Boolean) || [];
